...
# NAME: sort_photos_by_date.sh
# DESC: Scans a directory and sorts photos and videos into folders named "YYYY.MM.DD" based on capture date metadata, falling back to file modification date when metadata is missing

# Sort photos and videos into folders named YYYY.MM.DD based on:
# 1) Capture date (EXIF/QuickTime metadata) when available
# 2) File modification date as fallback
#
# Usage:
#   chmod +x sort_photos_by_date.sh
#   ./sort_photos_by_date.sh /path/to/dir

if [[ "$#" -ne 1 ]]; then
  echo "Error: you must provide exactly one folder path."
  echo ""
  echo "Usage:"
  echo './sort_photos_by_date.sh "/path/to/folder"'
  exit 1
fi
...
echo "Done: media has been sorted into YYYY.MM.DD folders."
